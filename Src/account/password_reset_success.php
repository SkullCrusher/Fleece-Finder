...
<?php if ($login->passwordResetLinkIsValid() == true) { ?>
	<form method="post" action="password_reset.php" name="new_password_form">
		...
		<input id="user_password_new" type="password" name="user_password_new" pattern=".{6,}" required autocomplete="off" class="textbox" pattern="[_a-zA-Z0-9]{2,64}" name="user_name" style="margin-left: 18px;margin-bottom: 20px"/>


		<label for="user_name" style="margin-left:33%">Repeat password</label>
		<input id="user_password_repeat" type="password" name="user_password_repeat" pattern=".{6,}" required autocomplete="off" class="textbox" pattern="[_a-zA-Z0-9]{2,64}" name="user_name" style="margin-bottom: 20px"/>
